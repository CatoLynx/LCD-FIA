...
def show_departures(dbi, ds100, fia, renderer, config, auto_clear_scroll_buf = False):
    ...
    layout_double = config.get('layout_double', {})
    layout_list = config.get('layout_list', {})
    ...
    if type(layout_list) is str:
        with open(layout_list, 'r', encoding='utf-8') as f:
    ...
    if mode == 'detail':
        ...
        for i in range(min(len(trains), 7)):
            data['placeholders']['departure_{}'.format(i+1)] = trains[i].get('scheduledDeparture')
            data['placeholders']['train_{}'.format(i+1)] = get_train_number(trains[i].get('train', ""), replacement_map)
            data['placeholders']['via_{}'.format(i+1)] = " \xb4 ".join(get_via(trains[i], replacement_map))
            data['placeholders']['destination_{}'.format(i+1)] = get_destination_name(trains[i].get('destination', ""), replacement_map)
            data['placeholders']['platform_{}'.format(i+1)] = str(trains[i].get('scheduledPlatform'))
            data['placeholders']['info_{}'.format(i+1)] = get_info_long(trains[i])
...
def main():
    ...
    parser.add_argument('--mode', '-t', choices=('detail', 'list'), default='list', type=str)
